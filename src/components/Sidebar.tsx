...
export default function Sidebar() {
  ...
  const [catOpen, setCatOpen] = useState(pathname.startsWith("/category"));
    ...
    <aside className="w-56 bg-white border-r border-gray-200 min-h-screen flex flex-col">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-green-50 text-green-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}>
              <span className={isActive ? "text-green-600" : "text-gray-400"}>{item.icon}</span>
              {item.label}
            </Link>
          );
        })}
        <button onClick={() => setCatOpen(!catOpen)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isCategoryPage ? "bg-green-50 text-green-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}>
        ...
        {catOpen && (
          <div className="ml-4 space-y-0.5">
            {CATEGORY_CODES.map((code) => {
              ...
              return (
                <Link key={code} href={catPath} className={`block px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${isActive ? "bg-green-50 text-green-700" : "text-gray-500 hover:bg-gray-50 hover:text-gray-800"}`}>
